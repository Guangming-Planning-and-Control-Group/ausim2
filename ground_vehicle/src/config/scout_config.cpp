...
void ApplyScoutConfigRoot(const YAML::Node& root, ScoutConfig* config) {
  ...
  if (bindings_node) {
    ...
    AssignIfPresent(
        wheel_actuators_node, "front_right", &config->bindings.wheel_actuators.front_right);
    AssignIfPresent(
        wheel_actuators_node, "front_left", &config->bindings.wheel_actuators.front_left);
    AssignIfPresent(
        wheel_actuators_node, "rear_left", &config->bindings.wheel_actuators.rear_left);
    AssignIfPresent(
        wheel_actuators_node, "rear_right", &config->bindings.wheel_actuators.rear_right);
  }
...
void ApplyScoutSpecificConfig(
  ...
  const std::optional<fs::path> robot_config_path =
      ResolveRobotConfigPath(root, absolute_config_path, explicit_robot_path);
